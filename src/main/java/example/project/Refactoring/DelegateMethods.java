...
public class DelegateMethods {
     ...
     * DoneTODO complete the method below using `delegate methods` refactoring technique.
     * DoneTODO: note that only the max parameter is used.
     *
     * @param max The maximum number (exclusive).
     * @return A random integer ranging from 0 to max.
     */
    public static int createRandomInteger(int max) {
        // DoneTODO: delegate the call to `createRandomValue()` method.
